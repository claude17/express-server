...
function getStringWidth$1(text) {
	...
	text = text.replace(emoji_regex_default$1(), (match) => narrowEmojisSet$1.has(match) ? " " : "  ");
	let width = 0;
...
function printDocToString$1(doc, options) {
	...
	while (commands.length > 0) {
		...
		switch (get_doc_type_default(doc2)) {
			...
			case DOC_TYPE_GROUP$1:
				switch (mode) {
					...
					case MODE_BREAK: {
						...
						if (!doc2.break && fits(next, commands, remainingWidth, hasLineSuffix, groupModeMap)) commands.push(next);
							...
							const mostExpanded = method_at_default$1(0, doc2.expandedStates, -1);
							if (doc2.break) {
								commands.push({
									indent: indent2,
									mode: MODE_BREAK,
				...
				if (doc2.id) groupModeMap[doc2.id] = method_at_default$1(0, commands, -1).mode;
				break;
			case DOC_TYPE_FILL$1: {
				const remainingWidth = width - position;
	...
	return {
		formatted,
		cursorNodeStart,
		cursorNodeText: formatted.slice(cursorNodeStart, method_at_default$1(0, finalCursorPositions, -1))
	};
	function trim2() {
...
var init_doc = __esmMin((() => {
	...
	method_at_default$1 = createMethodShim$1("at", function() {
	...
	method_replace_all_default$1 = createMethodShim$1("replaceAll", function() {
	...
	narrow_emojis_evaluate_default$1 = "©®‼⁉™ℹ↔↕↖↗↘↙↩↪⌨⏏⏱⏲⏸⏹⏺▪▫▶◀◻◼☀☁☂☃☄☎☑☘☝☠☢☣☦☪☮☯☸☹☺♀♂♟♠♣♥♦♨♻♾⚒⚔⚕⚖⚗⚙⚛⚜⚠⚧⚰⚱⛈⛏⛑⛓⛩⛱⛷⛸⛹✂✈✉✌✍✏✒✔✖✝✡✳✴❄❇❣❤➡⤴⤵⬅⬆⬇";
	notAsciiRegex$1 = /[^\x20-\x7F]/u;
	narrowEmojisSet$1 = new Set(narrow_emojis_evaluate_default$1);
...
function structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options8) {
	let optionsObj;
	if (!options8) optionsObj = {};
	else if (typeof options8 === "function") optionsObj = { callback: options8 };
	else optionsObj = options8;
...
function formatPatch(patch) {
	if (Array.isArray(patch)) return patch.map(formatPatch).join("\n");
	const ret = [];
	if (patch.oldFileName == patch.newFileName) ret.push("Index: " + patch.oldFileName);
	ret.push("===================================================================");
...
function createTwoFilesPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options8) {
	if (typeof options8 === "function") options8 = { callback: options8 };
	if (!(options8 === null || options8 === void 0 ? void 0 : options8.callback)) {
		...
		structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, Object.assign(Object.assign({}, options8), { callback: (patchObj) => {
			...
			else callback(formatPatch(patchObj));
		} }));
	}
}
...
function createMockable(implementations) {
	...
	const mockImplementation = (functionality, implementation) => {
		if (!Object.prototype.hasOwnProperty.call(implementations, functionality)) throw new Error(`Unexpected mock '${functionality}'.`);
...
async function findProjectRoot(startDirectory, options8) {
	...
	return directory ? path.dirname(directory) : void 0;
}
...
function highlight(text) {
	...
	let highlighted = "";
	for (const { type, value } of tokenize2(text)) if (type in defs) highlighted += value.split(NEWLINE$1).map((str) => defs[type](str)).join("\n");
...
function codeFrameColumns(rawLines, loc, opts = {}) {
	...
	let frame = (shouldHighlight ? highlight(rawLines) : rawLines).split(NEWLINE, end).slice(start, end).map((line3, index) => {
		...
		} else return ` ${defs.gutter(gutter)}${line3.length > 0 ? ` ${line3}` : ""}`;
	}).join("\n");
...
function parseString(str, ptr = 0, endPtr = str.length) {
	...
	while (ptr < endPtr - 1) {
		...
		if (isEscape) {
			...
			} else if (c2 in ESC_MAP) parsed += ESC_MAP[c2];
...
function parseValue(value, toml, ptr, integersAsBigInt) {
	...
	if (isInt2 || FLOAT_REGEX.test(value)) {
		...
		if (isInt2) {
			if ((isInt2 = !Number.isSafeInteger(numeric)) && !integersAsBigInt) throw new TomlError("integer value cannot be represented losslessly", {
				toml,
				ptr
			});
			if (isInt2 || integersAsBigInt === true) numeric = BigInt(value);
		}
		return numeric;
	}
...
function parseInlineTable(str, ptr, depth, integersAsBigInt) {
	...
	while ((c2 = str[ptr++]) !== "}" && c2) {
		...
		else if (c2 === "#") throw new TomlError("inline tables cannot contain comments", err);
		else if (c2 === ",") throw new TomlError("expected key-value, found comma", err);
		else if (c2 !== " " && c2 !== "	") {
			let k;
			let t = res;
			let hasOwn = false;
...
function getSearcher(stopDirectory) {
	return new FileSearcher(CONFIG_FILE_NAMES, {
		filter,
		stopDirectory
	});
}
...
function read2(jsonPath, { base, specifier }) {
	...
	let string;
	try {
		string = fs3.readFileSync(path10.toNamespacedPath(jsonPath), "utf8");
	} catch (error) {
...
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
	...
	process2.emitWarning(`Use of deprecated ${double ? "double slash" : "leading or trailing slash matching"} resolving "${target}" for module request "${request}" ${request === match ? "" : `matched to "${match}" `}in the "${internal ? "imports" : "exports"}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${fileURLToPath(base)}` : ""}.`, "DeprecationWarning", "DEP0166");
}
...
function loadEditorconfig2(file, options8) {
	if (!file || !options8.editorconfig) return;
...
function printDocToDebug(doc2) {
	...
	function printDoc(doc3, index, parentParts) {
		...
		if (doc3.type === DOC_TYPE_GROUP) {
			...
			const options8 = optionsParts.length > 0 ? `, { ${optionsParts.join(", ")} }` : "";
			if (doc3.expandedStates) return `conditionalGroup([${doc3.expandedStates.map((part) => printDoc(part)).join(",")}]${options8})`;
...
function getSortedChildNodes(node, ancestors, options8) {
	...
	if (childNodesCache2.has(node)) return childNodesCache2.get(node);
	const { filter: filter2 } = options8;
	if (!filter2) return [];
	let childAncestors;
	const childNodes = (options8.getChildren?.(node, options8) ?? [...getChildren(node, { getVisitorKeys: options8.getVisitorKeys })]).flatMap((child) => {
...
function decorateComment(node, comment, options8, enclosingNode, ancestors = []) {
	...
	while (left < right) {
		...
		if (start <= commentStart && commentEnd <= end) return decorateComment(child, comment, options8, child, [child, ...ancestors]);
		if (end <= commentStart) {
			precedingNode = child;
			left = middle + 1;
			continue;
	...
	if (enclosingNode?.type === "TemplateLiteral") {
		...
		if (precedingNode && findExpressionIndexForComment(quasis, precedingNode, options8) !== commentIndex) precedingNode = null;
		if (followingNode && findExpressionIndexForComment(quasis, followingNode, options8) !== commentIndex) followingNode = null;
	}
	return {
		enclosingNode,
		precedingNode,
...
function attachComments(ast, options8) {
	const { comments } = ast;
	delete ast.comments;
	if (!is_non_empty_array_default(comments) || !options8.printer.canAttachComment) return;
	const tiesToBreak = [];
	const { printer: { features: { experimental_avoidAstMutation: avoidAstMutation }, handleComments = {} }, originalText: text } = options8;
	...
	for (const [index, context] of decoratedComments.entries()) {
		...
		let args;
		if (avoidAstMutation) args = [context];
		else {
	...
	if (!avoidAstMutation) for (const comment of comments) {
...
function breakTies(tiesToBreak, options8) {
	...
	for (indexOfFirstLeadingComment = tieCount; indexOfFirstLeadingComment > 0; --indexOfFirstLeadingComment) {
		...
		if (options8.printer.isGap?.(gap, options8) ?? /^[\s(]*$/u.test(gap)) gapEndPos = options8.locStart(comment);
		else break;
	}
	...
	for (const node of [precedingNode, followingNode]) if (node.comments && node.comments.length > 1) node.comments.sort((a, b) => options8.locStart(a) - options8.locStart(b));
	tiesToBreak.length = 0;
}
function findExpressionIndexForComment(quasis, comment, options8) {
	const startPos = options8.locStart(comment) - 1;
	for (let i = 1; i < quasis.length; ++i) if (startPos < options8.locStart(quasis[i])) return i - 1;
	return 0;
}
...
function printCommentsSeparately(path15, options8) {
	...
	path15.each(() => {
		const comment = path15.node;
		if (ignored?.has(comment)) return;
		const { leading, trailing } = comment;
		if (leading) leadingParts.push(printLeadingComment(path15, options8));
		else if (trailing) {
...
function printComments(path15, doc2, options8) {
	...
	if (!leading && !trailing) return doc2;
	return inheritLabel(doc2, (doc3) => [
		leading,
		doc3,
		trailing
	]);
...
function getSupportInfo({ plugins = [], showDeprecated = false } = {}) {
	...
	for (const option of normalizeOptionSettings(Object.assign({}, ...plugins.map(({ options: options9 }) => options9), core_options_evaluate_default))) {
		...
		if (Array.isArray(option.choices)) {
			...
			if (option.name === "parser") option.choices = [...option.choices, ...collectParsersFromLanguages(option.choices, languages2, plugins)];
		}
...
function* collectParsersFromLanguages(parserChoices, languages2, plugins) {
	...
	for (const language of languages2) if (language.parsers) {
...
function getInterpreter(file) {
	try {
		...
		if (firstLineBuffer === false) return;
		liner.close();
...
function getLanguageByFileName(languages2, file) {
	...
	return languages2.find(({ filenames }) => filenames?.some((name) => name.toLowerCase() === basename)) ?? languages2.find(({ extensions }) => extensions?.some((extension) => basename.endsWith(extension)));
}
...
function getLanguageByInterpreterNodejs(languages2, file) {
	...
	const languagesWithInterpreters = languages2.filter(({ interpreters }) => is_non_empty_array_default(interpreters));
...
function inferParser(options8, fileInfo) {
	const languages2 = method_to_reversed_default(0, options8.plugins).flatMap((plugin) => plugin.languages ?? []);
...
async function printEmbedFrontMatter(textToDoc2, print, path15, options8) {
	...
	let doc2;
	if (value) {
		const parser = language === "yaml" ? language : infer_parser_default(options8, { language });
		if (!parser) return;
...
function getParserPluginByParserName(plugins, parserName) {
	...
	const plugin = method_find_last_default(0, plugins, (plugin2) => plugin2.parsers && Object.prototype.hasOwnProperty.call(plugin2.parsers, parserName));
...
function getPrinterPluginByAstFormat(plugins, astFormat) {
	...
	const plugin = method_find_last_default(0, plugins, (plugin2) => plugin2.printers && Object.prototype.hasOwnProperty.call(plugin2.printers, astFormat));
	if (plugin) return plugin;
...
function normalizePrinter(printer) {
	if (normalizedPrinters.has(printer)) return normalizedPrinters.get(printer);
	...
	if (frontMatterSupport.print) print = new Proxy(originalPrint, { apply(target, thisArgument, argumentsList) {
		...
		if (is_front_matter_default(path15.node)) return print_default(path15);
		return Reflect.apply(target, thisArgument, argumentsList);
	} });
...
async function normalizeFormatOptions(options8, opts = {}) {
	...
	if (!rawOptions.parser) if (!rawOptions.filepath) throw new UndefinedParserError("No parser and no file path given, couldn't infer a parser.");
	else {
	...
	for (const [k, value] of Object.entries(mixedDefaults)) if (rawOptions[k] === null || rawOptions[k] === void 0) rawOptions[k] = value;
...
async function parse5(originalText, options8) {
	...
	let ast;
	try {
		ast = await parser.parse(text, options8, options8);
	} catch (error) {
...
async function printEmbeddedLanguages(path15, genericPrint, options8, printAstToDoc2, embeds) {
	if (options8.embeddedLanguageFormatting !== "auto") return;
	...
	function recurse() {
		const { node } = path15;
		if (node === null || typeof node !== "object" || hasPrettierIgnore?.(path15)) return;
		for (const key2 of getVisitorKeys(node)) if (Array.isArray(node[key2])) path15.each(recurse, key2);
		else path15.call(recurse, key2);
...
async function printAstToDoc(ast, options8) {
	...
	if (options8.cursorOffset >= 0) {
		if (options8.nodeAfterCursor && !options8.nodeBeforeCursor) return [cursor, doc2];
		if (options8.nodeBeforeCursor && !options8.nodeAfterCursor) return [doc2, cursor];
	}
	return doc2;
	function mainPrint(selector, args) {
		if (selector === void 0 || selector === path15) return mainPrintInternal(args);
		if (Array.isArray(selector)) return path15.call(() => mainPrintInternal(args), ...selector);
	...
	function mainPrintInternal(args) {
		...
		if (shouldCache && cache3.has(value)) return cache3.get(value);
		const doc3 = callPluginPrintFunction(path15, options8, mainPrint, args, embeds);
		if (shouldCache) cache3.set(value, doc3);
		return doc3;
	}
}
function callPluginPrintFunction(path15, options8, printPath, args, embeds) {
	...
	const { printer } = options8;
	let doc2;
	if (printer.hasPrettierIgnore?.(path15)) doc2 = print_ignored_default(path15, options8, printPath, args);
	...
	if (printer.printComment && !printer.willPrintOwnComments?.(path15, options8)) doc2 = printComments(path15, doc2, options8);
	return doc2;
}
...
function massageAst(ast, options8) {
	...
	function recurse(original, parent) {
		...
		for (const key2 in original) {
			if (!Object.prototype.hasOwnProperty.call(original, key2) || ignoredProperties?.has(key2)) continue;
...
function calculateRange(text, opts, ast) {
	...
	if (!isAllWhitespace) {
		...
		for (; end > start; --end) if (/\S/u.test(text[end - 1])) break;
	}
	...
	const endNodeAndAncestors = isAllWhitespace ? startNodeAndAncestors : findNodeAtOffset(ast, end, opts, (node) => isSourceElement(opts, node), [], "rangeEnd");
	...
	} else [startNode, endNode] = findSiblingAncestors(startNodeAndAncestors, endNodeAndAncestors, opts);
...
async function coreFormat(originalText, opts, addAlignmentSize = 0) {
	...
	if (opts.cursorOffset >= 0) opts = {
		...opts,
		...get_cursor_node_default(ast, opts)
	};
	let doc2 = await printAstToDoc(ast, opts, addAlignmentSize);
	if (addAlignmentSize > 0) doc2 = addAlignmentToDoc([hardline3, doc2], addAlignmentSize, opts.tabWidth);
	...
	if (opts.cursorOffset >= 0) {
		...
		if ((opts.cursorNode || opts.nodeBeforeCursor || opts.nodeAfterCursor) && result.cursorNodeText) {
			...
			if (opts.cursorNode) {
				...
				if (!opts.nodeBeforeCursor && !opts.nodeAfterCursor) throw new Error("Cursor location must contain at least one of cursorNode, nodeBeforeCursor, nodeAfterCursor");
...
async function formatWithCursor(originalText, originalOptions) {
	...
	if (options8.rangeStart >= options8.rangeEnd && text !== "" || options8.requirePragma && !await hasPragma(text, options8) || options8.checkIgnorePragma && await hasIgnorePragma(text, options8)) return {
		formatted: originalText,
		cursorOffset: originalOptions.cursorOffset,
		comments: []
	};
	let result;
	if (options8.rangeStart > 0 || options8.rangeEnd < text.length) result = await formatRange(text, options8);
	else {
		if (!options8.requirePragma && options8.insertPragma && options8.printer.insertPragma && !await hasPragma(text, options8)) text = options8.printer.insertPragma(text);
...
async function parse6(originalText, originalOptions, devOptions) {
	...
	if (devOptions) {
		if (devOptions.preprocessForPrint) parsed.ast = await prepareToPrint(parsed.ast, options8);
		if (devOptions.massage) parsed.ast = massage_ast_default(parsed.ast, options8);
	}
	return parsed;
}
async function formatAst(ast, options8) {
	options8 = await normalize_format_options_default(options8);
	return printDocToStringWithoutNormalizeOptions(await printAstToDoc(ast, options8), options8);
}
...
async function printToDoc(originalText, options8) {
	...
	if (options8.cursorOffset >= 0) options8 = {
		...options8,
...
function createParsersAndPrinters(modules) {
	...
	for (const { importPlugin: importPlugin2, parsers: parserNames = [], printers: printerNames = [] } of modules) {
		...
		for (const parserName of parserNames) parsers2[parserName] = async () => (await loadPlugin2()).parsers[parserName];
		for (const printerName of printerNames) printers2[printerName] = async () => (await loadPlugin2()).printers[printerName];
	}
	return {
		parsers: parsers2,
		printers: printers2
...
function loadPlugin(plugin) {
	...
	if (!cache2.has(cacheKey)) cache2.set(cacheKey, loadPluginWithoutCache(plugin, cwd));
...
async function getFileInfo(file, options8 = {}) {
	...
	let inferredParser;
	if (!ignored) inferredParser = options8.parser ?? await getParser(file, options8);
	return {
		ignored,
...
async function getParser(file, options8) {
	let config;
	if (options8.resolveConfig !== false) config = await resolveConfig(file, { editorconfig: false });
...
var init_prettier = __esmMin((() => {
	...
	require_is_glob = __commonJS({ "node_modules/is-glob/index.js"(exports, module) {
		...
		module.exports = function isGlob(str, options8) {
			...
			if (options8 && options8.strict === false) check2 = relaxedCheck;
			return check2(str);
		};
	} });
	...
	require_to_regex_range = __commonJS({ "node_modules/to-regex-range/index.js"(exports, module) {
		...
		function rangeToPattern(start, stop, options8) {
			...
			for (let i = 0; i < digits; i++) {
				...
				else if (startDigit !== "0" || stopDigit !== "9") pattern += toCharacterClass(startDigit, stopDigit, options8);
				else count++;
			}
			if (count) pattern += options8.shorthand === true ? "\\d" : "[0-9]";
			return {
				pattern,
				count: [count],
				digits
		...
		function splitToPatterns(min, max, tok, options8) {
			...
			for (let i = 0; i < ranges.length; i++) {
				...
				if (tok.isPadded) zeros = padZeros(max2, tok, options8);
	...
	require_fill_range = __commonJS({ "node_modules/fill-range/index.js"(exports, module) {
		...
		var toSequence = (parts, options8, maxLen) => {
			...
			if (options8.wrap) return `(${prefix}${result})`;
			return result;
		};
		...
		var rangeError = (...args) => {
			return /* @__PURE__ */ new RangeError("Invalid range arguments: " + util2.inspect(...args));
		};
		...
		var invalidStep = (step, options8) => {
			if (options8.strictRanges === true) throw new TypeError(`Expected step "${step}" to be a number`);
			return [];
		};
		var fillNumbers = (start, end, step = 1, options8 = {}) => {
			...
			if (options8.toRegex && step === 1) return toRange(toMaxLen(start, maxLen), toMaxLen(end, maxLen), true, options8);
			let parts = {
				negatives: [],
				positives: []
			};
			...
			while (descending ? a >= b : a <= b) {
				if (options8.toRegex === true && step > 1) push2(a);
				else range.push(pad(format3(a, index), maxLen, toNumber));
				a = descending ? a - step : a + step;
				index++;
			}
			if (options8.toRegex === true) return step > 1 ? toSequence(parts, options8, maxLen) : toRegex(range, null, {
				wrap: false,
				...options8
			});
			return range;
		...
		var fillLetters = (start, end, step = 1, options8 = {}) => {
			if (!isNumber(start) && start.length > 1 || !isNumber(end) && end.length > 1) return invalidRange(start, end, options8);
			...
			if (options8.toRegex && step === 1) return toRange(min, max, false, options8);
			let range = [];
			let index = 0;
			while (descending ? a >= b : a <= b) {
			...
			if (options8.toRegex === true) return toRegex(range, null, {
				wrap: false,
				options: options8
			});
			return range;
		...
		var fill = (start, end, step, options8 = {}) => {
			...
			if (!isValidValue(start) || !isValidValue(end)) return invalidRange(start, end, options8);
	...
	require_expand = __commonJS({ "node_modules/braces/lib/expand.js"(exports, module) {
		...
		var expand = (ast, options8 = {}) => {
			...
			const walk = (node, parent = {}) => {
				...
				if (node.nodes && node.ranges > 0) {
					const args = utils.reduce(node.nodes);
					if (utils.exceedsLimit(...args, options8.step, rangeLimit)) throw new RangeError("expanded array length exceeds range limit. Use options.rangeLimit to increase or disable the limit.");
					let range = fill(...args, options8);
					if (range.length === 0) range = stringify2(node, options8);
		...
		var parse7 = (input, options8 = {}) => {
			...
			while (index < length) {
				...
				if (value === CHAR_DOUBLE_QUOTE || value === CHAR_SINGLE_QUOTE || value === CHAR_BACKTICK) {
					const open = value;
					let next;
					if (options8.keepQuotes !== true) value = "";
					while (index < length && (next = advance())) {
						if (next === CHAR_BACKSLASH) {
							value += next + advance();
							continue;
	...
	require_braces = __commonJS({ "node_modules/braces/index.js"(exports, module) {
		...
		var braces = (input, options8 = {}) => {
			...
			else output = [].concat(braces.create(input, options8));
			if (options8 && options8.expand === true && options8.nodupes === true) output = [...new Set(output)];
			return output;
		};
		braces.parse = (input, options8 = {}) => parse7(input, options8);
		braces.stringify = (input, options8 = {}) => {
			if (typeof input === "string") return stringify2(braces.parse(input, options8), options8);
			return stringify2(input, options8);
		};
		braces.compile = (input, options8 = {}) => {
			if (typeof input === "string") input = braces.parse(input, options8);
			return compile(input, options8);
		};
		braces.expand = (input, options8 = {}) => {
			if (typeof input === "string") input = braces.parse(input, options8);
			let result = expand(input, options8);
			if (options8.noempty === true) result = result.filter(Boolean);
			if (options8.nodupes === true) result = [...new Set(result)];
			return result;
		};
	...
	require_utils2 = __commonJS({ "node_modules/micromatch/node_modules/picomatch/lib/utils.js"(exports) {
		...
		exports.isWindows = (options8) => {
			if (options8 && typeof options8.windows === "boolean") return options8.windows;
	...
	require_parse2 = __commonJS({ "node_modules/micromatch/node_modules/picomatch/lib/parse.js"(exports, module) {
		...
		var expandRange = (args, options8) => {
			if (typeof options8.expandRange === "function") return options8.expandRange(...args, options8);
			args.sort();
		...
		var parse7 = (input, options8) => {
			...
			while (!eos()) {
				...
				if (value === ")") {
					if (state.parens === 0 && opts.strictBrackets === true) throw new SyntaxError(syntaxError2("opening", "("));
				...
				if (value === "[") {
					if (opts.nobracket === true || !remaining().includes("]")) {
						if (opts.nobracket !== true && opts.strictBrackets === true) throw new SyntaxError(syntaxError2("closing", "]"));
						value = `\\${value}`;
				...
				if (value === "]") {
					...
					if (state.brackets === 0) {
						if (opts.strictBrackets === true) throw new SyntaxError(syntaxError2("opening", "["));
						push2({
							type: "text",
							value,
							output: `\\${value}`
			...
			if (opts.strictSlashes !== true && (prev.type === "star" || prev.type === "bracket")) push2({
				type: "maybe_slash",
				value: "",
				output: `${SLASH_LITERAL}?`
			});
	...
	require_picomatch = __commonJS({ "node_modules/micromatch/node_modules/picomatch/lib/picomatch.js"(exports, module) {
		...
		picomatch.test = (input, regex, options8, { glob, posix } = {}) => {
			...
			if (match === false || opts.capture === true) if (opts.matchBase === true || opts.basename === true) match = picomatch.matchBase(input, regex, options8, posix);
		...
		picomatch.matchBase = (input, glob, options8, posix = utils.isWindows(options8)) => {
			return (glob instanceof RegExp ? glob : picomatch.makeRe(glob, options8)).test(path15.basename(input));
		};
		...
		picomatch.makeRe = (input, options8 = {}, returnOutput = false, returnState = false) => {
			...
			if (options8.fastpaths !== false && (input[0] === "." || input[0] === "*")) parsed.output = parse7.fastpaths(input, options8);
			if (!parsed.output) parsed = parse7(input, options8);
	...
	require_micromatch = __commonJS({ "node_modules/micromatch/index.js"(exports, module) {
		...
		var micromatch2 = (list, patterns, options8) => {
			...
			let onResult = (state) => {
				...
				if (options8 && options8.onResult) options8.onResult(state);
			};
		...
		micromatch2.contains = (str, pattern, options8) => {
			if (typeof str !== "string") throw new TypeError(`Expected a string: "${util2.inspect(str)}"`);
			if (Array.isArray(pattern)) return pattern.some((p) => micromatch2.contains(str, p, options8));
		...
		micromatch2.parse = (patterns, options8) => {
			let res = [];
			for (let pattern of [].concat(patterns || [])) for (let str of braces(String(pattern), options8)) res.push(picomatch.parse(str, options8));
			return res;
		};
		micromatch2.braces = (pattern, options8) => {
			if (typeof pattern !== "string") throw new TypeError("Expected a string");
			if (options8 && options8.nobrace === true || !hasBraces(pattern)) return [pattern];
	...
	require_pattern = __commonJS({ "node_modules/fast-glob/out/utils/pattern.js"(exports) {
		...
		function isDynamicPattern(pattern, options8 = {}) {
			...
			if (options8.caseSensitiveMatch === false || pattern.includes(ESCAPE_SYMBOL)) return true;
			...
			if (options8.extglob !== false && GLOB_EXTENSION_SYMBOLS_RE.test(pattern)) return true;
			if (options8.braceExpansion !== false && hasBraceExpansion(pattern)) return true;
			return false;
		}
	...
	require_merge2 = __commonJS({ "node_modules/merge2/index.js"(exports, module) {
		...
		function merge2() {
			...
			if (options8 && !Array.isArray(options8) && options8.pipe == null) args.pop();
			else options8 = {};
			const doEnd = options8.end !== false;
			const doPipeError = options8.pipeError === true;
			if (options8.objectMode == null) options8.objectMode = true;
			if (options8.highWaterMark == null) options8.highWaterMark = 64 * 1024;
			const mergedStream = PassThrough(options8);
			function addStream() {
				for (let i = 0, len = arguments.length; i < len; i++) streamsQueue.push(pauseStreams(arguments[i], options8));
				mergeStream();
				return this;
			}
		...
		function pauseStreams(streams, options8) {
			if (!Array.isArray(streams)) {
				if (!streams._readableState && streams.pipe) streams = streams.pipe(PassThrough(options8));
				if (!streams._readableState || !streams.pause || !streams.pipe) throw new Error("Only readable stream can be merged.");
				streams.pause();
			} else for (let i = 0, len = streams.length; i < len; i++) streams[i] = pauseStreams(streams[i], options8);
			return streams;
		}
	} });
	...
	require_queue = __commonJS({ "node_modules/fastq/queue.js"(exports, module) {
		...
		function fastqueue(context, worker, _concurrency) {
			...
			function release(holder) {
				if (holder) cache3.release(holder);
				var next = queueHead;
		...
		var ReaderStream = class extends reader_1.default {
			...
			static(patterns, options8) {
				...
				stream._write = (index, _enc, done) => {
					return this._getEntry(filepaths[index], patterns[index], options8).then((entry) => {
						if (entry !== null && options8.entryFilter(entry)) stream.push(entry);
			...
			_getEntry(filepath, pattern, options8) {
				return this._getStat(filepath).then((stats) => this._makeEntry(stats, pattern)).catch((error) => {
					if (options8.errorFilter(error)) return null;
					throw error;
				});
			}
			_getStat(filepath) {
			...
			dynamic(root2, options8) {
				return new Promise((resolve3, reject) => {
					this._walkAsync(root2, options8, (error, entries) => {
						if (error === null) resolve3(entries);
						else reject(error);
					});
				});
			}
		...
		var ReaderSync = class extends reader_1.default {
			...
			static(patterns, options8) {
				...
				for (const pattern of patterns) {
					...
					if (entry === null || !options8.entryFilter(entry)) continue;
					entries.push(entry);
				}
				return entries;
			}
	...
	require_debug = __commonJS({ "node_modules/semver/internal/debug.js"(exports, module) {
		"use strict";
		module.exports = typeof process === "object" && process.env && process.env.NODE_DEBUG && /\bsemver\b/i.test(process.env.NODE_DEBUG) ? (...args) => console.error("SEMVER", ...args) : () => {};
	} });
	require_constants4 = __commonJS({ "node_modules/semver/internal/constants.js"(exports, module) {
		"use strict";
	...
	require_re = __commonJS({ "node_modules/semver/internal/re.js"(exports, module) {
		"use strict";
	...
	require_parse_options = __commonJS({ "node_modules/semver/internal/parse-options.js"(exports, module) {
		"use strict";
		var looseOption = Object.freeze({ loose: true });
		var emptyOpts = Object.freeze({});
		var parseOptions = (options8) => {
			if (!options8) return emptyOpts;
			if (typeof options8 !== "object") return looseOption;
			return options8;
		};
		module.exports = parseOptions;
	} });
	require_identifiers = __commonJS({ "node_modules/semver/internal/identifiers.js"(exports, module) {
		"use strict";
		var numeric = /^[0-9]+$/;
	...
	require_semver = __commonJS({ "node_modules/semver/classes/semver.js"(exports, module) {
		"use strict";
		var debug = require_debug();
	...
	require_compare = __commonJS({ "node_modules/semver/functions/compare.js"(exports, module) {
		"use strict";
		var SemVer = require_semver();
		var compare = (a, b, loose) => new SemVer(a, loose).compare(new SemVer(b, loose));
		module.exports = compare;
	} });
	require_gte = __commonJS({ "node_modules/semver/functions/gte.js"(exports, module) {
		"use strict";
		var compare = require_compare();
		var gte = (a, b, loose) => compare(a, b, loose) >= 0;
		module.exports = gte;
	} });
	require_pseudomap = __commonJS({ "node_modules/pseudomap/pseudomap.js"(exports, module) {
		var hasOwnProperty3 = Object.prototype.hasOwnProperty;
		module.exports = PseudoMap;
		function PseudoMap(set2) {
			if (!(this instanceof PseudoMap)) throw new TypeError("Constructor PseudoMap requires 'new'");
			this.clear();
			if (set2) if (set2 instanceof PseudoMap || typeof Map === "function" && set2 instanceof Map) set2.forEach(function(value, key2) {
				this.set(key2, value);
			}, this);
		...
		function find(data, k) {
			for (var i = 0, s = "_" + k, key2 = s; hasOwnProperty3.call(data, key2); key2 = s + i++) if (same(data[key2].key, k)) return data[key2];
		}
		function set(data, k, v) {
			for (var i = 0, s = "_" + k, key2 = s; hasOwnProperty3.call(data, key2); key2 = s + i++) if (same(data[key2].key, k)) {
				data[key2].value = v;
				return;
			}
			data.size++;
	...
	require_map = __commonJS({ "node_modules/pseudomap/map.js"(exports, module) {
		...
		else module.exports = require_pseudomap();
	} });
	require_yallist = __commonJS({ "node_modules/yallist/yallist.js"(exports, module) {
		...
		Yallist.prototype.push = function() {
			for (var i = 0, l = arguments.length; i < l; i++) push2(this, arguments[i]);
			return this.length;
		};
	...
	require_lru_cache = __commonJS({ "node_modules/editorconfig/node_modules/lru-cache/index.js"(exports, module) {
		"use strict";
		...
		var makeSymbol;
		if (hasSymbol) makeSymbol = function(key2) {
			return Symbol(key2);
		};
		else makeSymbol = function(key2) {
			return "_" + key2;
		};
		...
		function LRUCache(options8) {
			if (!(this instanceof LRUCache)) return new LRUCache(options8);
			if (typeof options8 === "number") options8 = { max: options8 };
		...
		LRUCache.prototype.set = function(key2, value, maxAge) {
			...
			if (hit.length > this[MAX]) {
				if (this[DISPOSE]) this[DISPOSE](key2, value);
				return false;
			}
			this[LENGTH] += hit.length;
	...
	require_sigmund = __commonJS({ "node_modules/sigmund/sigmund.js"(exports, module) {
		...
		function sigmund(subject, maxSessions) {
			...
			function psychoAnalyze(subject2, session) {
				...
				if (typeof subject2 === "function" || typeof subject2 === "undefined") return;
	...
	require_fnmatch = __commonJS({ "node_modules/editorconfig/src/lib/fnmatch.js"(exports, module) {
		...
		function minimatch(p, pattern, options8) {
			...
			if (!options8) options8 = {};
			if (!options8.nocomment && pattern.charAt(0) === "#") return false;
			if (pattern.trim() === "") return p === "";
			return new Minimatch(pattern, options8).match(p);
		}
		function Minimatch(pattern, options8) {
			if (!(this instanceof Minimatch)) return new Minimatch(pattern, options8, cache3);
			if (typeof pattern !== "string") throw new TypeError("glob pattern string required");
			if (!options8) options8 = {};
		...
		function braceExpand(pattern, options8) {
			...
			if (options8.nobrace || !pattern.match(/\{.*\}/)) return [pattern];
			var escaping = false;
			if (pattern.charAt(0) !== "{") {
				var prefix = null;
				for (var i = 0, l = pattern.length; i < l; i++) {
					var c2 = pattern.charAt(i);
					if (c2 === "\\") escaping = !escaping;
					else if (c2 === "{" && !escaping) {
			...
			if (depth !== 0) return braceExpand("\\" + pattern, options8);
		...
		function parse7(pattern, isSub) {
			...
			for (var i = 0, len = pattern.length, c2; i < len && (c2 = pattern.charAt(i)); i++) {
				if (options8.debug) console.error("%s	%s %s %j", pattern, i, re, c2);
				...
				switch (c2) {
					...
					case "@":
					case "!":
						if (options8.debug) console.error("%s	%s %s %j <-- stateChar", pattern, i, re, c2);
						if (inClass) {
					...
					default:
						...
						else if (reSpecials[c2] && !(c2 === "^" && inClass)) re += "\\";
						re += c2;
				}
			}
			if (inClass) {
		...
		minimatch.match = function(list, pattern, options8) {
			...
			if (options8.nonull && !list.length) list.push(pattern);
			return list;
		};
		...
		function match(f, partial) {
			...
			if (options8.debug) console.error(this.pattern, "split", f);
			var set = this.set;
		...
		Minimatch.prototype.matchOne = function(file, pattern, partial) {
			...
			if (options8.matchBase && pattern.length === 1) file = path15.basename(file.join("/")).split("/");
			if (options8.debug) console.error("matchOne", file.length, pattern.length);
			for (var fi = 0, pi = 0, fl = file.length, pl = pattern.length; fi < fl && pi < pl; fi++, pi++) {
				if (options8.debug) console.error("matchOne loop");
				var p = pattern[pi], f = file[fi];
				if (options8.debug) console.error(pattern, p, f);
				if (p === false) return false;
				if (p === GLOBSTAR) {
					if (options8.debug) console.error("GLOBSTAR", [
						pattern,
						p,
						f
					]);
					...
					if (pr === pl) {
						...
						for (; fi < fl; fi++) if (file[fi] === "." || file[fi] === ".." || !options8.dot && file[fi].charAt(0) === ".") return false;
						return true;
					}
					WHILE: while (fr < fl) {
				...
				var hit;
				if (typeof p === "string") {
					if (options8.nocase) hit = f.toLowerCase() === p.toLowerCase();
					else hit = f === p;
					if (options8.debug) console.error("string match", p, f, hit);
				} else {
					hit = f.match(p);
					if (options8.debug) console.error("pattern match", p, f, hit);
				}
				if (!hit) return false;
			}
	...
	require_ini = __commonJS({ "node_modules/editorconfig/src/lib/ini.js"(exports) {
		"use strict";
	...
	require_package = __commonJS({ "node_modules/editorconfig/package.json"(exports, module) {
		module.exports = {
			name: "editorconfig",
			version: "0.15.3",
	...
	require_src = __commonJS({ "node_modules/editorconfig/src/index.js"(exports) {
		"use strict";
		var __awaiter = exports && exports.__awaiter || function(thisArg, _arguments, P, generator) {
			return new (P || (P = Promise))(function(resolve3, reject) {
				function fulfilled(value) {
		...
		function processMatches(matches, version) {
			if ("indent_style" in matches && matches.indent_style === "tab" && !("indent_size" in matches) && semver.gte(version, "0.10.0")) matches.indent_size = "tab";
		...
		function extendProps(props, options8) {
			...
			if (options8 === void 0) options8 = {};
			for (var key2 in options8) if (options8.hasOwnProperty(key2)) {
				var value = options8[key2];
				var key22 = key2.toLowerCase();
				var value2 = value;
				if (knownProps[key22]) value2 = value.toLowerCase();
				try {
					value2 = JSON.parse(value);
				} catch (e) {}
				if (typeof value === "undefined" || value === null) value2 = String(value);
				props[key22] = value2;
			}
			return props;
		}
	...
	require_js_tokens = __commonJS({ "node_modules/@babel/code-frame/node_modules/js-tokens/index.js"(exports, module) {
		var Identifier;
		var JSXIdentifier;
		var JSXPunctuator;
		var JSXString;
	...
	require_readlines = __commonJS({ "node_modules/n-readlines/readlines.js"(exports, module) {
		"use strict";
		var fs4 = __require("fs");
		var LineByLine = class {
			constructor(file, options8) {
				...
				if (!options8.newLineCharacter) options8.newLineCharacter = 10;
				else options8.newLineCharacter = options8.newLineCharacter.charCodeAt(0);
			...
			_extractLines(buffer2) {
				...
				while (true) {
					...
					} else if (bufferPositionValue === void 0) break;
				}
			...
			_readChunk(lineLeftovers) {
				...
				if (totalBytesRead) {
					...
					if (lineLeftovers) this.linesCache[0] = Buffer.concat([lineLeftovers, this.linesCache[0]]);
				}
			...
			next() {
				...
				if (line3 && line3[line3.length - 1] === this.newLineCharacter) line3 = line3.slice(0, line3.length - 1);
				return line3;
			}
		};
	...
	require_ignore = __commonJS({ "node_modules/ignore/index.js"(exports, module) {
		...
		var RuleManager = class {
			...
			test(path15, checkUnignored, mode) {
				...
				this._rules.forEach((rule) => {
					...
					if (!rule[mode].test(path15)) return;
					ignored = !negative;
					unignored = negative;
		...
		var checkPath = (path15, originalPath, doThrow) => {
			if (!isString(path15)) return doThrow(`path must be a string, but got \`${originalPath}\``, TypeError);
			if (!path15) return doThrow(`path must not be empty`, TypeError);
			if (checkPath.isNotRelative(path15)) return doThrow(`path should be a \`path.relative()\`d string, but got "${originalPath}"`, RangeError);
			return true;
		};
		...
		var Ignore = class {
			...
			checkIgnore(path15) {
				if (!REGEX_TEST_TRAILING_SLASH.test(path15)) return this.test(path15);
			...
			_t(path15, cache3, checkUnignored, slices) {
				if (path15 in cache3) return cache3[path15];
				if (!slices) slices = path15.split(SLASH).filter(Boolean);
				slices.pop();
				if (!slices.length) return cache3[path15] = this._rules.test(path15, checkUnignored, MODE_IGNORE);
	...
	Diff = class {
		diff(oldStr, newStr, options8 = {}) {
			...
			if (typeof options8 === "function") {
				callback = options8;
				options8 = {};
			} else if ("callback" in options8) callback = options8.callback;
		...
		diffWithOptionsObj(oldTokens, newTokens, options8, callback) {
			...
			if (options8.maxEditLength != null) maxEditLength = Math.min(maxEditLength, options8.maxEditLength);
			...
			const execEditLength = () => {
				for (let diagonalPath = Math.max(minDiagonalToConsider, -editLength); diagonalPath <= Math.min(maxDiagonalToConsider, editLength); diagonalPath += 2) {
					...
					if (!canRemove || canAdd && removePath.oldPos < addPath.oldPos) basePath = this.addToPath(addPath, true, false, 0, options8);
					else basePath = this.addToPath(removePath, false, true, 1, options8);
		...
		addToPath(path15, added, removed, oldPosInc, options8) {
			...
			if (last && !options8.oneChangePerToken && last.added === added && last.removed === removed) return {
		...
		extractCommon(basePath, newTokens, oldTokens, diagonalPath, options8) {
			...
			while (newPos + 1 < newLen && oldPos + 1 < oldLen && this.equals(oldTokens[oldPos + 1], newTokens[newPos + 1], options8)) {
				...
				oldPos++;
				commonCount++;
				if (options8.oneChangePerToken) basePath.lastComponent = {
					count: 1,
			...
			if (commonCount && !options8.oneChangePerToken) basePath.lastComponent = {
				count: commonCount,
				previousComponent: basePath.lastComponent,
				added: false,
				removed: false
		...
		equals(left, right, options8) {
			if (options8.comparator) return options8.comparator(left, right);
			else return left === right || !!options8.ignoreCase && left.toLowerCase() === right.toLowerCase();
		}
		removeEmpty(array2) {
			const ret = [];
	...
	VALUE_NOT_EXIST = Symbol.for("vnopts.VALUE_NOT_EXIST");
	VALUE_UNCHANGED = Symbol.for("vnopts.VALUE_UNCHANGED");
	...
	Normalizer = class {
		...
		_applyRequiredCheck(options8) {
			for (const key2 of Object.keys(this._utils.schemas)) if (this._identifyMissing(key2, options8)) {
		...
		_applyPostprocess(options8) {
			...
			if (postprocessed.delete) for (const deleteKey of postprocessed.delete) delete options8[deleteKey];
	...
	toAbsolutePath = (urlOrPath) => urlOrPath ? path.resolve(isUrl(urlOrPath) ? url.fileURLToPath(urlOrPath) : urlOrPath) : urlOrPath;
	...
	Searcher = class {
		...
		*/
		constructor(nameOrNames, { allowSymlinks, filter: filter2, stopDirectory, cache: cache3 } = {}) {
			...
			this.#searchWithoutCache = (directory) => this.findInDirectory(nameOrNames, {
				cwd: directory,
				filter: filter2,
				allowSymlinks
			});
		...
		#search(directory, cache3 = true) {
			...
			if (!cache3 || !resultCache.has(directory)) resultCache.set(directory, this.#searchWithoutCache(directory));
		...
		*/
		async search(startDirectory, options8) {
			for (const directory of iterate_directory_up_default(startDirectory, this.#stopDirectory)) {
				const result = await this.#search(directory, options8?.cache ?? this.#cache);
	...
	parse2 = function parse3(text, reviver) {
		source = String(text);
		parseState = "start";
		stack = [];
		pos = 0;
	...
	new RegExp("[" + nonASCIIidentifierStartChars + "]");
	...
	compose = (f, g) => (v) => f(g(v));
	defsOn = buildDefs((0, import_picocolors4.createColors)(true));
	defsOff = buildDefs((0, import_picocolors4.createColors)(false));
	...
	BRACKET = /^[()[\]{}]$/;
	{
		const getTokenType = function(token2) {
			if (token2.type === "IdentifierName") {
				if (isKeyword(token2.value) || isStrictReservedWord(token2.value, true) || sometimesKeywords.has(token2.value)) return "keyword";
				if (token2.value[0] !== token2.value[0].toLowerCase()) return "capitalized";
			}
		...
		tokenize2 = function* (text) {
			for (const token2 of (0, import_js_tokens.default)(text, { jsx: true })) switch (token2.type) {
				case "TemplateHead":
					yield {
						type: "string",
...
${codeblock}`, options8);
	...
	method_replace_all_default = createMethodShim("replaceAll", function() {
	...
	END_OF_LINE_REGEXP = /\r\n?/gu;
	method_at_default = createMethodShim("at", function() {
	...
	({breakParent, hardline, indent, join: join3, line: line2, lineSuffix} = builders);
	...
	method_to_reversed_default = createMethodShim("toReversed", function() {
		if (Array.isArray(this)) return arrayToReversed;
	});
	import_n_readlines = __toESM(require_readlines(), 1);
	...
	method_find_last_default = createMethodShim("findLast", function() {
		if (Array.isArray(this)) return arrayFindLast;
	});
	FRONT_MATTER_MARK = Symbol.for("PRETTIER_IS_FRONT_MATTER");
	...
	method_find_last_index_default = createMethodShim("findLastIndex", function() {
		if (Array.isArray(this)) return arrayFindLastIndex;
	});
	isJsonParser = ({ parser }) => parser === "json" || parser === "json5" || parser === "jsonc" || parser === "json-stringify";
	...
	options_default = { singleQuote: common_options_evaluate_default.singleQuote };
	...
	options_default2 = { bracketSpacing: common_options_evaluate_default.bracketSpacing };
	...
	({parsers, printers} = createParsersAndPrinters([
		...
		{
			importPlugin: () => import("./babel-Cs312VeV.js"),
			parsers: [
				"babel",
				"babel-flow",
				"babel-ts",
	...
	builtin_plugins_proxy_default = [estreePlugin, {
		options: options7,
		languages,
		parsers,
		printers
	...
	inferParser2 = withPlugins((file, options8) => infer_parser_default(options8, { physicalFile: file }));
	sharedWithCli = {
